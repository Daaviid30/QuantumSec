...
import { formatIdentifier, formatTechnicalText } from '../../lib/labels'
    ...
    <aside className="surface composition-summary">
      ...
      {profile.supports_qkd ? (
        <div className="composition-flow">
          ...
          <div className="composition-node"><Atom size={15} /><span><strong>Quantum channel</strong><small>{qkdStages.length ? qkdStages.map((stage) => formatIdentifier(stage.type)).join(' → ') : 'Ideal pipeline'}</small></span></div>
          <ArrowDown size={14} className="composition-arrow" />
      ...
      <div className="composition-block">
        ...
        {profile.authentication.map((item) => <p key={item}><ShieldCheck size={13} /> {formatTechnicalText(item)}</p>)}
      </div>
      <div className="composition-block">
