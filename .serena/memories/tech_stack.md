...
- Runtime Python: NumPy `>=2.5`, Matplotlib `>=3.11`, Rich `>=15`, SymPy `>=1.14`, tqdm `>=4.68.3`, FastAPI `>=0.115`, Pydantic `>=2.10`, Uvicorn `>=0.34`.
...
- Pyright: modo `basic`, Python 3.14, entorno `.venv`, alcance `core/ quantum/ qkd/ ui/backend/ tests/`.
